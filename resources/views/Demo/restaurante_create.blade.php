...
<head>
	...
	<style>
		...
		.success { color:#2f8e44; font-size:.95rem; margin-bottom:1rem; background:#f0f8f5; padding:.6rem .8rem; border-radius:8px; }
		@media (max-width:900px) {
			.restaurante-card { grid-template-columns: 1fr; }
	...
	<div class="restaurante-container">
		<div class="restaurante-card">
			<div class="side-brand">
				...
				<p class="primary">Registra un nuevo restaurante en tu plataforma. Configura los datos básicos y asigna un administrador para comenzar.</p>
				<div style="display:flex;flex-direction:column;gap:.6rem;margin-top:1rem;">
					<div style="display:flex;gap:.6rem;align-items:center;">
						<span class="material-icons-sharp" style="background:rgba(255,255,255,0.12);padding:.4rem;border-radius:.5rem;">storefront</span>
						<span>Gestión completa</span>
					</div>
					<div style="display:flex;gap:.6rem;align-items:center;">
						<span class="material-icons-sharp" style="background:rgba(255,255,255,0.12);padding:.4rem;border-radius:.5rem;">people</span>
			...
			<div class="restaurante-right">
				<form method="POST" action="{{ route('restaurante.store') }}">
					...
					<label class="field" for="nombre" style="margin-top:1rem;">
						...
						<input id="nombre" type="text" name="nombre" placeholder="Nombre del restaurante" value="{{ old('nombre') }}" required autofocus>
					</label>
					...
					<label class="field" for="direccion" style="margin-top:.6rem;">
						...
						<input id="direccion" type="text" name="direccion" placeholder="Dirección" value="{{ old('direccion') }}" required>
					</label>
					...
					<label class="field" for="telefono" style="margin-top:.6rem;">
						...
						<input id="telefono" type="text" name="telefono" placeholder="Teléfono" value="{{ old('telefono') }}" required>
					</label>
					...
					<label class="field" for="fecha_creacion" style="margin-top:.6rem;">
						...
						<input id="fecha_creacion" type="date" name="fecha_creacion" value="{{ old('fecha_creacion') }}">
					</label>
					@error('fecha_creacion') <div class="error">{{ $message }}</div> @enderror

					<!-- Administrador -->
					<label class="field" for="rut_admin" style="margin-top:.6rem;">
						<span class="material-icons-sharp">person</span>
						<input id="rut_admin" type="text" name="rut_admin" placeholder="RUT del administrador (ej: 12.345.678-9)" value="{{ old('rut_admin') }}" required>
					</label>
					@error('rut_admin') <div class="error">{{ $message }}</div> @enderror
					...
					<div style="margin-top:1rem;display:flex;justify-content:center;gap:.5rem;align-items:center;">
						...
						<a href="{{ route('index') }}" class="link-muted" style="font-weight:600;">Ir al panel</a>
					</div>
				</form>
			</div>
		</div>
